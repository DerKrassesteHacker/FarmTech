...
void loop(){
  ...
  if(millis()-averagingTime > 5000){  //Every 5 Seconds
  ...
  if(millis()-pump_intervall > 120000){  //time interval: 2min
    if (ec_value <= minimal_ec_value){
      pump_time = millis();
      fert_pump_status = true;
      digitalWrite(FERT_PUMP_PIN, HIGH);
    }else if (ph_value >= 6.30){
      pump_time = millis();
      ph_pump_status = true;
...
void measureData(){
  ...
  if(millis() - watertempTime > 0.75){  //time interval: 0.75s
  ...
  if (millis() - ecAndpHTime > 500 && millis() - ecAndpHTime < 2500){  //EC- and pH-Sensor taking turn measuring data to avoid interference
    ...
    if(millis() - ecTime > 30 && (millis() - ecAndpHTime) > 3000){  //time interval: 30ms
      ...
      watertemp = 20;
      float ec_value = ec.readEC(ec_voltage, 20);  // convert voltage to EC with temperature compensation
      ec_values.push_back(ec_value);
      //Serial.print("EC-Wert: "); Serial.println(ec_value);
...
void averageData(){
  ...
  air_temp_vals.clear();
  air_temp_stats.push_back(air_temp);
  ...
  Serial.print("Temperature: "); Serial.print(air_temp); Serial.println(" C"); //Output to the console


  humidity = avgValue(humidity_vals);
  humidity_vals.clear();
  humidity_stats.push_back(humidity);
  ...
  Serial.print("Wasser über Wasserstandssensor? "); Serial.println(waterlevel);
...
void statisticalData(){
  ...
  Serial.print("Temperatur: "); Serial.print(avgValue(air_temp_stats)); Serial.println(" C"); 
  ...
  air_temp_stats.clear();

  Serial.print("Luftfeuchtigkeit: "); Serial.print(avgValue(humidity_stats)); Serial.println(" %"); 
  ...
  humidity_stats.clear();

  Serial.print("Wassertemperatur: "); Serial.print(avgValue(watertemp_stats)); Serial.println(" C"); 
  ...
  watertemp_stats.clear();

  Serial.print("ph-Wert: "); Serial.println(avgValue(ph_stats)); 
  ...
  Serial.print("EC-Wert: "); Serial.print(avgValue(ec_stats)); Serial.println(" ms/cm"); 
  ...
  Serial.print("Licht-Wert: "); Serial.println(avgValue(light_stats));
